  ...
  return (
    <div style={{ width: '100%' }}>
      <Button
        ...
        sx={{ width: '100%' }}
      >
        {files.length ? `Add More Images (${files.length}/5)` : 'Upload Images'}
        <input
          hidden
          type='file'
          accept='image/*'
      ...
      <FormError error={error} style={{ background: 'white', padding: 10, borderRadius: 5, marginTop: 8 }} />

      {files.length > 0 && (
        <div className={styles.previews}>
          {previews.map((url, index) => (
            <div key={url} className={styles.preview}>
              <img src={url} alt={files[index]?.name || `image-${index}`} className={styles.thumb} />
              <IconButton
                size='small'
